...
class RateLimitConfig:
    ...
    def __init__(self):
        ...
        # Default rate limit
        self.default_max_requests_per_second = 2
        
        # API-specific rate limits from settings
        # Convert RPM to RPS (requests per minute / 60)
        self.tavily_rate_limit = settings.tavily_rpm / 60.0
        self.gemini_rate_limit = settings.gemini_rpm / 60.0
        self.newsapi_rate_limit = settings.newsapi_rpm / 60.0
...
def get_rate_limiter(
    ...
    Args:
        api_type: Type of API (e.g., "tavily", "gemini")
    ...
    if max_requests_per_second is None:
        if api_type == "tavily":
            max_requests_per_second = int(config.tavily_rate_limit)
        elif api_type == "gemini":
            max_requests_per_second = int(config.gemini_rate_limit)
        elif api_type == "newsapi":
            max_requests_per_second = int(config.newsapi_rate_limit)
        else:
